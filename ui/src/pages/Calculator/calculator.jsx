          ...
          {expanded===exc.id&&(
            <div style={{padding:"12px",background:"#f9fafb",borderTop:"1px solid #e5e7eb"}}>
              ...
              <div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center",marginBottom:14}}>
                ...
                <input value={newCond.value} onChange={e=>setNewCond(p=>({...p,value:_ld(e.target.value,20)}))} placeholder="Значение" maxLength={20} style={{...inp,width:110,padding:"4px 7px",fontSize:12}}/>
                {newCond.op==="between"&&<><span style={{fontSize:12,color:"#6b7280"}}>до</span><input value={newCond.value2} onChange={e=>setNewCond(p=>({...p,value2:_ld(e.target.value,20)}))} placeholder="До" maxLength={20} style={{...inp,width:70,padding:"4px 7px",fontSize:12}}/></>}
...
function MgrRatesPanel({cfg,onSave}){
  ...
  return(
    <div style={{fontSize:13}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:14,background:"#eff6ff",border:"1px solid #bfdbfe",borderRadius:8,padding:"10px 14px"}}>
        <span style={{fontSize:13,fontWeight:700,color:"#1d4ed8",whiteSpace:"nowrap"}}>📅 Менеджер работает с:</span>
        <input type="month" value={local.managerStartDate||""} onChange={e=>setLocal(p=>({...p,managerStartDate:e.target.value}))} style={{...inp,padding:"5px 8px",fontSize:13}}/>
...
function RnPolModal({p,onClose,rnTabs,rnActiveId,parseAnyDate,getName}){
  ...
  const disp=s=>{if(!s)return"—";const d=parseAnyDate(s);if(!d)return String(s);return d.toLocaleDateString("ru-RU");};
...
export default function App(){
  ...
  const[auditFilter,setAuditFilter]=useState({user:"",category:"",dateFrom:"",dateTo:"",search:""});
  const[amexTopups,setAmexTopups]=useState([]);
  const[amexAllPols,setAmexAllPols]=useState([]);
...
try{const r=await calcStorage.get("officeCodes:"+selMonth).catch(()=>null);if(r&&r.value)setOfficeCodes(JSON.parse(r.value));else setOfficeCodes([]);}catch{setOfficeCodes([]);}
  ...
  const isMreoPol=p=>p._mreo===true||(mreoAgentUid!==null&&p.agentUid===mreoAgentUid);
  ...
  const saveAppSettings=(updates={})=>{
    if(!_settingsLoadedRef.current)return;
  ...
  const loadChartData=async()=>{
    ...
    try{
      ...
      for(const m of months){
        ...
        if(or?.value)JSON.parse(or.value).forEach(p=>{if(!p.insuredName?.includes("ПРИМЕР")&&!isMreoPol(p))result[m].office+=Number(p.amount)||0;});
  ...
  const loadComparison=async()=>{
    ...
    try{
      ...
      const loadM=async(m)=>{
        ...
        const or=await calcStorage.get("officePol:"+m).catch(()=>null);
        if(or?.value){
          const pols=JSON.parse(or.value).filter(p=>!p.insuredName?.includes("ПРИМЕР")&&!isMreoPol(p));
        ...
        if(ar?.value){
          ...
          [...(dd.policies||[]),...(dd.voluntary||[])].forEach(p=>{
            const amt=Number(p.amount)||0;
            if(mgrUids.has(p.agentUid)){r.manager+=amt;r.managerCount++;}
            else{r.agents+=amt;r.agentCount++;}
          });
        }
        r.expenses=(officeExpenses[m]||[]).reduce((s,row)=>s+(Number(row.amount)||0),0);
        return r;
      };
      const[d1,d2]=await Promise.all([loadM(cmpMonth1),loadM(cmpMonth2)]);
      setCmpData({m1:cmpMonth1,m2:cmpMonth2,d1,d2});
    }catch{setCmpData(null);}
    setCmpLoading(false);
  };
  const saveOfficeStaff=(list)=>{if(!_settingsLoadedRef.current)return;const prev=officeStaff;_officeStaffRef.current=list;setOfficeStaff(list);const s={adminPin:_adminPinRef.current,employees:_employeesRef.current,officeStaff:list};calcStorage.set("appSettings",JSON.stringify(s)).catch(err=>{_officeStaffRef.current=prev;setOfficeStaff(prev);_saveErr("сотрудники офиса")(err);});};
  ...
  const saveEmployees=(list)=>{if(!_settingsLoadedRef.current)return;const prev=employees;_employeesRef.current=list;setEmployees(list);const s={adminPin:_adminPinRef.current,employees:list,officeStaff:_officeStaffRef.current};calcStorage.set("appSettings",JSON.stringify(s)).catch(err=>{_employeesRef.current=prev;setEmployees(prev);_saveErr("сотрудники")(err);});logAction("settings","Изменён список сотрудников","—");};
  ...
  const parseAnyDate=s=>{if(s==null||s==="")return null;try{if(typeof s==="string"){if(s.includes("T"))return new Date(s);if(/^\d{4}-\d{2}-\d{2}$/.test(s)){const[y,mo,d]=s.split("-");return new Date(+y,+mo-1,+d);}if(/^\d{2}\.\d{2}\.\d{4}$/.test(s)){const[d,mo,y]=s.split(".");return new Date(+y,+mo-1,+d);}}const d=new Date(s);return isNaN(d)?null:d;}catch{return null;}};
  ...
  const checkRenewals=async(tid)=>{
    ...
    try{
      ...
      let expiring=[];
      if(subTab==="office"){
        for(const mk of scanMonths){const r=await calcStorage.get(`officePol:${mk}`).catch(()=>null);if(!r?.value)continue;JSON.parse(r.value).filter(p=>{if(p.insuredName?.includes("ПРИМЕР"))return false;if(isMreoPol(p))return false;if(p.term==="SH")return false;if(!p.carPlate)return false;const d=parseAnyDate(p.dateEnd);return d&&d>=pStart&&d<=pEnd;}).forEach(p=>expiring.push({...p,_mk:mk}));}
      }else{
      ...
      for(const mk of searchMonths){
        ...
        if(ro?.value)JSON.parse(ro.value).filter(p=>!p.insuredName?.includes("ПРИМЕР")&&p.carPlate).forEach(p=>cands.push({...p,_np:normPlate(p.carPlate),_ds:parseAnyDate(p.dateStart),_src:"office",_mk:mk}));
        const ra=await calcStorage.get(`month:${mk}`).catch(()=>null);
        if(ra?.value)(JSON.parse(ra.value).policies||[]).filter(p=>p.carPlate).forEach(p=>cands.push({...p,_np:normPlate(p.carPlate),_ds:parseAnyDate(p.startDate),_src:"agent",_mk:mk}));
      }
  ...
  const clearCompanyData=async(co)=>{
    ...
    if(co==="voluntary"){
      ...
      await calcStorage.set("month:"+selMonth,JSON.stringify(newData)).catch(_saveErr("удаление данных компании"));
  ...
  const lockMonth=async()=>{
    ...
    await calcStorage.set("lockedMonths",JSON.stringify(updated)).catch(_saveErr("блокировка месяца"));
  ...
  const exportDetailedIncomeXlsx=()=>{
    ...
    // ── Sheet: Менеджер — ОСАГО ─────────────────────────────────
    const mkMgrOsagoSheet=(rows)=>{
      const ws={};const cfg=managerConfig;
      hdr(ws,['#','Агент','Код','№ полиса','Страхователь','Компания','Ставка офиса %','Ставка менеджера %','Статус / Ограничение','Страховая премия ֏','Доход офиса ֏  (=премия×ставка офиса%)','Выплата менеджеру ֏  (=премия×ставка мен.%)','Прибыль офиса ֏  (=доход−выплата)']);
      let tAmt=0,tOff=0,tMgr=0,tPrf=0,ri=1;
      rows.forEach(({uid,policies})=>{
        policies.forEach(p=>{
          const exc=!!p.exception,ev=(ri-1)%2===0,rn=ri+1;
          const reason=exc?excReason(p,effExceptions,uid)||'Исключён':'Зачёт';
          const mgrRate=exc?0:getMgrPolicyRate(p,cfg);
          const mgrPay=exc?0:Math.round(p.amount*mgrRate/100);
          tAmt+=p.amount||0;tOff+=p.officeComm||0;tMgr+=mgrPay;tPrf+=(p.officeComm||0)-mgrPay;
          cel(ws,ri,0,ri,getS(exc,ev,'n'));
          cel(ws,ri,1,gName(uid),getS(exc,ev,'b'));
          cel(ws,ri,2,gCode(uid),getS(exc,ev));
          cel(ws,ri,3,p.policyNum||'',getS(exc,ev,'b'));
          ...
          cel(ws,ri,5,detectCo(p.company)||p.company||'',getS(exc,ev));
          cel(ws,ri,6,p.officeRate||0,getS(exc,ev,'n'));
          cel(ws,ri,7,mgrRate,getS(exc,ev,'n'));
          cel(ws,ri,8,reason,getS(exc,ev));
          cel(ws,ri,9,p.amount||0,getS(exc,ev,'n'));
          cel(ws,ri,10,p.officeComm||0,getS(exc,ev,'f'),exc?`0`:`J${rn}*G${rn}/100`);
          cel(ws,ri,11,mgrPay,getS(exc,ev,'f'),exc?`0`:`J${rn}*H${rn}/100`);
          cel(ws,ri,12,(p.officeComm||0)-mgrPay,getS(exc,ev,'f'),`K${rn}-L${rn}`);
          ri++;
        });
      });
      const rt=ri;
      totRow(ws,rt,13,[{c:9,v:tAmt},{c:10,v:tOff,formula:true},{c:11,v:tMgr},{c:12,v:tPrf,formula:true}]);
      ws[EC({r:rt,c:9})].f=`SUM(J2:J${rt})`;ws[EC({r:rt,c:10})].f=`SUM(K2:K${rt})`;ws[EC({r:rt,c:11})].f=`SUM(L2:L${rt})`;ws[EC({r:rt,c:12})].f=`SUM(M2:M${rt})`;
      ws['!ref']=ER({s:{r:0,c:0},e:{r:rt,c:12}});
      ws['!cols']=[5,22,12,14,30,12,14,16,28,18,28,28,28].map(w=>({wch:w}));
      return ws;
    };
    ...
    if(mgrRows.some(r=>r.policies.length))XLSXStyle.utils.book_append_sheet(wb,mkMgrOsagoSheet(mgrRows),'Менеджер — ОСАГО');
  ...
  const exportAgentAll=(uid,validPols,excPols,volPols,month)=>{
    ...
    const ER=XLSXStyle.utils.encode_range;
    const NCOLS=13;
    const agInfo=agentDir[uid];
    const agName=agInfo?(agInfo.name+' '+agInfo.surname).trim():uid;
    const agCode=agInfo?.internalCode||uid;
    const br={style:'thin',color:{rgb:'D1D5DB'}};
    const brd={top:br,bottom:br,left:br,right:br};
    const mkS=(bg,bold,fg,al,sz)=>({fill:bg?{patternType:'solid',fgColor:{rgb:bg}}:{patternType:'none'},font:{bold:!!bold,color:{rgb:fg||'1E293B'},name:'Calibri',sz:sz||10},border:brd,alignment:{horizontal:al||'left',vertical:'center'}});
    const ws={};const merges=[];let rw=0;
    const cel=(r,c,v,s,f)=>{const o={v:v??'',s,t:typeof v==='number'?'n':'s'};if(f)o.f=f;ws[EC({r,c})]=o;};
    const secRow=(r,text,s)=>{cel(r,0,text,s);for(let c=1;c<NCOLS;c++)cel(r,c,'',s);merges.push({s:{r,c:0},e:{r,c:NCOLS-1}});};
    // Title
    secRow(rw,`Начисления — ${agName}  (${agCode}) — ${fmtMonth(month)}`,mkS('1E3A5F',true,'FFFFFF','left',12));rw+=2;
    // ── ЗАЧЁТНЫЕ ──────────────────────────────────────────────────────
    const sGs=mkS('166534',true,'FFFFFF','left',11);
    const sGH=mkS('DCFCE7',true,'15803D','center');
    const sGHn=mkS('DCFCE7',true,'15803D','right');
    const sGe=mkS('F0FDF4',false,'1E293B');const sGo=mkS(null,false,'1E293B');
    const sGne=mkS('F0FDF4',false,'1E293B','right');const sGno=mkS(null,false,'1E293B','right');
    const sGfe=mkS('DCFCE7',false,'15803D','right');const sGfo=mkS(null,false,'15803D','right');
    const sGce=mkS('F0FDF4',false,'1E293B','center');const sGco=mkS(null,false,'1E293B','center');
    const sGTl=mkS('BBF7D0',true,'166534');const sGTn=mkS('BBF7D0',true,'166534','right');const sGTf=mkS('86EFAC',true,'166534','right');
    secRow(rw,`✓  ЗАЧЁТНЫЕ ПОЛИСЫ — ${validPols.length} шт.`,sGs);rw++;
    ["#","№ полиса","Компания","Страхователь","Марка","Рег. номер","Начало","Окончание","Регион","БМ","Сумма (֏)","Ставка %","Начислено (֏)"]
      .forEach((h,c)=>cel(rw,c,h,c>=9?sGHn:sGH));rw++;
    const vR0=rw;
    validPols.forEach((p,i)=>{const ev=i%2===0;const er=rw+1;
      cel(rw,0,i+1,ev?sGne:sGno);cel(rw,1,p.policyNum||'',ev?sGe:sGo);cel(rw,2,p.company||'',ev?sGe:sGo);
      cel(rw,3,p.insuredName||'',ev?sGe:sGo);cel(rw,4,p.car||'',ev?sGe:sGo);cel(rw,5,p.carPlate||'',ev?sGe:sGo);
      cel(rw,6,p.startDateFmt||'',ev?sGe:sGo);cel(rw,7,p.endDateFmt||'',ev?sGe:sGo);
      cel(rw,8,p.region||'',ev?sGce:sGco);cel(rw,9,p.bm||'',ev?sGce:sGco);
      cel(rw,10,p.amount||0,ev?sGne:sGno);cel(rw,11,p.agentRate||0,ev?sGne:sGno);
      cel(rw,12,p.agentComm||0,ev?sGfe:sGfo,`K${er}*L${er}/100`);rw++;});
    if(validPols.length===0){cel(rw,0,'Нет зачётных полисов',sGe);for(let c=1;c<NCOLS;c++)cel(rw,c,'',sGe);rw++;}
    const vR1=rw-1;
    cel(rw,0,'ИТОГО',sGTl);for(let c=1;c<10;c++)cel(rw,c,'',sGTl);
    cel(rw,10,validPols.reduce((s,p)=>s+p.amount,0),sGTn,validPols.length?`SUM(K${vR0+1}:K${vR1+1})`:undefined);
    cel(rw,11,'',sGTl);
    cel(rw,12,validPols.reduce((s,p)=>s+p.agentComm,0),sGTf,validPols.length?`SUM(M${vR0+1}:M${vR1+1})`:undefined);rw+=2;
    // ── НЕЗАЧЁТНЫЕ ────────────────────────────────────────────────────
    if(excPols.length>0){
      const sRs=mkS('B91C1C',true,'FFFFFF','left',11);
      const sRH=mkS('FEE2E2',true,'991B1B','center');const sRHn=mkS('FEE2E2',true,'991B1B','right');
      const sRe=mkS('FFF5F5',false,'7F1D1D');const sRo=mkS(null,false,'7F1D1D');
      const sRne=mkS('FFF5F5',false,'991B1B','right');const sRno=mkS(null,false,'991B1B','right');
      const sRce=mkS('FFF5F5',false,'7F1D1D','center');const sRco=mkS(null,false,'7F1D1D','center');
      const sRTl=mkS('FCA5A5',true,'991B1B');const sRTn=mkS('FCA5A5',true,'991B1B','right');
      secRow(rw,`✗  НЕЗАЧЁТНЫЕ ПОЛИСЫ — ${excPols.length} шт.`,sRs);rw++;
      ["#","№ полиса","Компания","Страхователь","Марка","Рег. номер","Начало","Окончание","Регион","БМ","Сумма (֏)","Причина исключения",""]
        .forEach((h,c)=>cel(rw,c,h,c===10?sRHn:sRH));rw++;
      const eR0=rw;
      excPols.forEach((p,i)=>{const ev=i%2===0;const reason=excReason(p,effExceptions,uid)||'Исключён';
        cel(rw,0,i+1,ev?sRne:sRno);cel(rw,1,p.policyNum||'',ev?sRe:sRo);cel(rw,2,p.company||'',ev?sRe:sRo);
        cel(rw,3,p.insuredName||'',ev?sRe:sRo);cel(rw,4,p.car||'',ev?sRe:sRo);cel(rw,5,p.carPlate||'',ev?sRe:sRo);
        cel(rw,6,p.startDateFmt||'',ev?sRe:sRo);cel(rw,7,p.endDateFmt||'',ev?sRe:sRo);
        cel(rw,8,p.region||'',ev?sRce:sRco);cel(rw,9,p.bm||'',ev?sRce:sRco);
        cel(rw,10,p.amount||0,ev?sRne:sRno);cel(rw,11,reason,ev?sRe:sRo);cel(rw,12,'',ev?sRe:sRo);rw++;});
      const eR1=rw-1;
      cel(rw,0,'ИТОГО',sRTl);for(let c=1;c<10;c++)cel(rw,c,'',sRTl);
      cel(rw,10,excPols.reduce((s,p)=>s+p.amount,0),sRTn,`SUM(K${eR0+1}:K${eR1+1})`);
      cel(rw,11,'',sRTl);cel(rw,12,'',sRTl);rw+=2;
    }
    // ── ДОБРОВОЛЬНЫЕ ─────────────────────────────────────────────────
    if(volPols&&volPols.length>0){
      const sBs=mkS('1D4ED8',true,'FFFFFF','left',11);
      const sBH=mkS('DBEAFE',true,'1E40AF','center');const sBHn=mkS('DBEAFE',true,'1E40AF','right');
      const sBe=mkS('EFF6FF',false,'1E293B');const sBo=mkS(null,false,'1E293B');
      const sBne=mkS('EFF6FF',false,'1E293B','right');const sBno=mkS(null,false,'1E293B','right');
      const sBfe=mkS('DBEAFE',false,'1D4ED8','right');const sBfo=mkS(null,false,'1D4ED8','right');
      const sBTl=mkS('BFDBFE',true,'1E40AF');const sBTn=mkS('BFDBFE',true,'1E40AF','right');const sBTf=mkS('93C5FD',true,'1D4ED8','right');
      secRow(rw,`🛡  ДОБРОВОЛЬНЫЕ СТРАХОВКИ — ${volPols.length} шт.`,sBs);rw++;
      ["#","№ полиса","Продукт","Компания","Страхователь","Сумма (֏)","Ставка %","Начислено (֏)","К оплате офису (֏)","","","",""]
        .forEach((h,c)=>cel(rw,c,h,c>=5&&c<=8?sBHn:sBH));rw++;
      const bR0=rw;
      volPols.forEach((p,i)=>{const ev=i%2===0;const er=rw+1;
        cel(rw,0,i+1,ev?sBne:sBno);cel(rw,1,p.policyNum||'',ev?sBe:sBo);cel(rw,2,p.productName||'',ev?sBe:sBo);
        cel(rw,3,p.company||'',ev?sBe:sBo);cel(rw,4,p.insuredName||'',ev?sBe:sBo);
        cel(rw,5,p.amount||0,ev?sBne:sBno);cel(rw,6,p.agentRate||0,ev?sBne:sBno);
        cel(rw,7,p.agentComm||0,ev?sBfe:sBfo,`F${er}*G${er}/100`);
        cel(rw,8,(p.amount||0)-(p.agentComm||0),ev?sBne:sBno,`F${er}-H${er}`);
        for(let c=9;c<NCOLS;c++)cel(rw,c,'',ev?sBe:sBo);rw++;});
      const bR1=rw-1;
      cel(rw,0,'ИТОГО',sBTl);for(let c=1;c<5;c++)cel(rw,c,'',sBTl);
      cel(rw,5,volPols.reduce((s,v)=>s+v.amount,0),sBTn,`SUM(F${bR0+1}:F${bR1+1})`);
      cel(rw,6,'',sBTl);
      cel(rw,7,volPols.reduce((s,v)=>s+v.agentComm,0),sBTf,`SUM(H${bR0+1}:H${bR1+1})`);
      cel(rw,8,volPols.reduce((s,v)=>s+(v.amount-v.agentComm),0),sBTn,`SUM(I${bR0+1}:I${bR1+1})`);
      for(let c=9;c<NCOLS;c++)cel(rw,c,'',sBTl);rw++;
    }
    ws['!ref']=ER({s:{r:0,c:0},e:{r:Math.max(rw-1,0),c:NCOLS-1}});
    ws['!cols']=[{wch:5},{wch:14},{wch:10},{wch:28},{wch:16},{wch:12},{wch:11},{wch:11},{wch:8},{wch:5},{wch:14},{wch:24},{wch:14}];
    ws['!merges']=merges;
    XLSXStyle.utils.book_append_sheet(wb,ws,`Начисления ${agCode}`.slice(0,31));
    _dlXlsx(wb,`Начисления_${agCode}_${month}.xlsx`);
  };
    ...
    <div style={{fontFamily:"system-ui,-apple-system,'Segoe UI',Roboto,sans-serif",padding:20,maxWidth:1400,margin:"0 auto",color:"#1e293b",background:"#dde3ed",minHeight:"100vh"}}>
            ...
            <div>
              ...
              <div style={{background:"white",border:"1px solid #e5e7eb",borderRadius:8,padding:12,marginBottom:12}}>
                ...
                <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:8}}>
                  <input value={newIC} onChange={e=>setNewIC(_ascii(e.target.value,16))} placeholder="Внутр. код" maxLength={16} style={{...inp,width:110,padding:"4px 8px",fontWeight:600,color:"#6366f1"}}/>
                  <input value={newName} onChange={e=>setNewName(e.target.value.slice(0,20))} placeholder="Имя" maxLength={20} style={{...inp,width:120,padding:"4px 8px"}}/>
                  <input value={newSur} onChange={e=>setNewSur(e.target.value.slice(0,20))} placeholder="Фамилия" maxLength={20} style={{...inp,width:140,padding:"4px 8px"}}/>
                </div>
                <div style={{fontSize:12,color:"#6b7280",marginBottom:6}}>Коды по компаниям:</div>
                <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:10}}>
                  {ALL_COMPANIES.map(c=><div key={c} style={{display:"flex",flexDirection:"column",gap:3}}><span style={{fontSize:11,color:"#6b7280",fontWeight:600}}>{c}</span><input value={newCodes[c]||""} onChange={e=>setNewCodes(p=>({...p,[c]:_ascii(e.target.value,20)}))} placeholder="Код" maxLength={20} style={{...inp,width:80,padding:"3px 6px",fontSize:12}}/></div>)}
                </div>
                ...
                <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
                  ...
                  <tbody>{Object.entries(agentDir).map(([aUid,ag])=>{
                    ...
                    return(<tr key={aUid}>{editUid===aUid?<>
                      <td style={td}><input defaultValue={ic} id={"ic-"+aUid} onInput={e=>{e.target.value=_ascii(e.target.value,20);}} maxLength={20} style={{...inp,width:80,padding:"3px 7px",fontWeight:600,color:"#6366f1"}}/></td>
                      <td style={td}><input defaultValue={name} id={"n-"+aUid} maxLength={20} style={{...inp,width:90,padding:"3px 7px"}}/></td>
          ...
          {panel==="access"&&(
            <div style={{fontSize:13}}>
              ...
              <div style={{display:"flex",gap:8,alignItems:"center"}}>
                <input value={newStaffCode} onChange={e=>setNewStaffCode(e.target.value.slice(0,20))} onKeyDown={e=>e.key==="Enter"&&(newStaffCode.trim()&&!officeStaff.includes(newStaffCode.trim()))&&(saveOfficeStaff([...officeStaff,newStaffCode.trim()]),setNewStaffCode(""))} placeholder="Внутренний код (напр. 768-101)" maxLength={20} style={{...inp,width:220,padding:"6px 10px"}}/>
          ...
          <div style={{border:"1px solid #e5e7eb",borderRadius:10,padding:14,marginBottom:12,background:"#fafafa"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))",gap:8}}>
              {[...ALL_COMPANIES,"voluntary"].map(co=>{
                ...
                const logoMap={Nairi:"/logos/nairi.png",Ingo:"/logos/ingo.png",Liga:"/logos/liga.png",Sil:"/logos/sil.png",Rego:"/logos/rego.png",Armenia:"/logos/armenia.png"};
                const logoSrc=!isVol&&logoMap[co]?logoMap[co]:null;
                ...
                return(
                  <div key={co} onClick={()=>handleSlotClick(co)}
                    ...
                    {logoSrc
                      ?<img src={logoSrc} alt={co} style={{height:32,maxWidth:"100%",objectFit:"contain",marginBottom:2}}/>
                    ...
                    {logoSrc&&state!=="empty"&&<div style={{position:"absolute",top:6,right:6,fontSize:12}}>{styles.icon}</div>}
                    {state==="session"&&(
                      <div>
              ...
              {(()=>{const paidLock=!!(opEditPol?.paid);const effectiveLock=paidLock&&!isAdmin;const amtLock=!!opEditPol&&!isAdmin;const lk=(lock)=>lock?{background:"#f1f5f9",color:"#1e293b",cursor:"not-allowed"}:{};const fe=(key)=>opFormErrors.includes(key)?{border:"2px solid #dc2626"}:{};return(<>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
                    ...
                    <div>
                      ...
                      <div style={{display:"flex",gap:6,opacity:effectiveLock?0.6:1,pointerEvents:effectiveLock?"none":"auto"}}>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
                    <div>
                      ...
                      <input value={opFD.car||""} onChange={e=>setOpFD(p=>({...p,car:_ld(e.target.value,20)}))} placeholder="Toyota Camry..." maxLength={20} disabled={effectiveLock} style={{...finp,width:"100%",boxSizing:"border-box",...lk(effectiveLock)}}/>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <select value={opFD.region||""} onChange={e=>setOpFD(p=>({...p,region:e.target.value}))} disabled={effectiveLock} style={{...finp,width:"100%",boxSizing:"border-box",...lk(effectiveLock)}}>
                    ...
                    <div>
                      ...
                      <input type="text" value={opFD.power||""} onChange={e=>setOpFD(p=>({...p,power:_dig(e.target.value,4)}))} placeholder="л.с." maxLength={4} disabled={effectiveLock} style={{...finp,width:"100%",boxSizing:"border-box",...lk(effectiveLock)}}/>
                    </div>
                  </div>
                  ...
                  {/* === Источник оплаты (только ОСАГО) === */}
                  {opFD.polType==="osago"&&(
                  <div style={{marginBottom:14}}>
                    <div style={{fontSize:11,fontWeight:700,color:"#374151",marginBottom:6,textTransform:"uppercase",letterSpacing:.5}}>Источник оплаты</div>
                    <div style={{display:"flex",gap:6}}>
                      {[[false,"🏦 Другой источник"],[true,"💳 Amex"]].map(([val,label])=>(
                        <button key={String(val)} type="button" onClick={()=>setOpFD(p=>({...p,paid_from_amex:val}))}
                          style={{...btn(opFD.paid_from_amex===val?(val?"#1d4ed8":"#374151"):"#f3f4f6",opFD.paid_from_amex===val?"#fff":"#374151",{border:"2px solid "+(opFD.paid_from_amex===val?(val?"#1d4ed8":"#374151"):"#e5e7eb"),fontSize:12,padding:"6px 14px"})}}>
                          {label}
                        </button>
                      ))}
                    </div>
                  ...
                  {opFD.polType==="voluntary"&&!paidLock&&(
                  ...
                  <div style={{marginBottom:16}}>
                    ...
                    <input value={opFD.comment||""} onChange={e=>setOpFD(p=>({...p,comment:e.target.value.slice(0,150)}))} onKeyDown={e=>e.key==="Enter"&&submitOpForm()} placeholder="Необязательно" maxLength={150} style={{...finp,width:"100%",boxSizing:"border-box"}}/>
                  </div>
                  {paidLock&&opEditPol&&(
                    <div style={{background:"#f0fdf4",border:"1px solid #86efac",borderRadius:8,padding:"10px 14px",marginBottom:14,fontSize:12,color:"#166534"}}>
                      <div style={{fontWeight:700,marginBottom:6}}>✓ Данные об оплате</div>
                      <div style={{display:"flex",alignItems:"center",gap:16,flexWrap:"wrap"}}>
                        <span>Сумма: <strong>{fmt(opEditPol.paidAmount||0)}</strong></span>
                        <span>Дата: <strong>{fmtPolDate(opEditPol.paidDate)}</strong></span>
                        <span style={{display:"flex",alignItems:"center",gap:6}}>
                          Способ:{isAdmin
                            ?<select value={opFD.paymentType||""} onChange={e=>setOpFD(p=>({...p,paymentType:e.target.value}))} style={{...finp,padding:"2px 6px",fontSize:12,fontWeight:700,color:"#166534",background:"#dcfce7",border:"1px solid #86efac",borderRadius:6,cursor:"pointer"}}>
                      ...
                      {isAdmin&&<div style={{fontSize:11,color:"#15803d",marginTop:6}}>Нажмите «Сохранить изменения» чтобы применить.</div>}
                    </div>
                  )}
      ...
      {tab==="cashbook"&&(()=>{
        ...
        const _relevantPols=isMreoMode
          ?cashMonthPols.filter(isMreoPol)
          :cashMonthPols.filter(p=>!isMreoPol(p));
        const byDay={};
        ...
        return(
          <div>
            ...
            {(isAdmin&&employees.some(e=>e.cashMode==="mreo"))&&(
        ...
        <div>
          ...
          <div style={{border:"1px solid #e5e7eb",borderRadius:8,padding:"12px 14px",marginBottom:20,background:"#fafafa"}}>
            ...
            <div style={{display:"flex",gap:8,alignItems:"center"}}>
              <input
                value={newOfficeCode}
                onChange={e=>setNewOfficeCode(_ld(e.target.value,20))}
      ...
      {tab==="manager"&&(()=>{
        ...
        const mgrActive=!cfg.managerStartDate||selMonth>=cfg.managerStartDate;
        const opResults=mgrActive?_computeOpR(agentData,cfg):[];
        ...
        return(
          <div>
            ...
            {/* Manager inactive banner */}
            {!mgrActive&&(
              <div style={{background:"#fef3c7",border:"1px solid #fcd34d",borderRadius:8,padding:"12px 16px",marginBottom:16,fontSize:13,color:"#92400e",fontWeight:600}}>
                ⚠ Менеджер не работал в {fmtMonth(selMonth)} — активен с {fmtMonth(cfg.managerStartDate)}. Расчёты не применяются.
              </div>
            )}
            {/* Summary cards */}
            {opResults.length>0&&(
      ...
      {tab==="income"&&(()=>{
        ...
        const iS={
          ...
          thL:{padding:"7px 12px",fontSize:11,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.05em",color:"#e2e8f0",background:"#1e293b",textAlign:"left",whiteSpace:"nowrap",borderBottom:"1px solid #334155"},
          thR:{padding:"7px 12px",fontSize:11,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.05em",color:"#e2e8f0",background:"#1e293b",textAlign:"right",whiteSpace:"nowrap",borderBottom:"1px solid #334155"},
        ...
        const rowEl=r=>(
          <div key={r.id} style={{display:"flex",alignItems:"center",gap:8,padding:"5px 14px",borderBottom:"1px solid #f9fafb"}}>
            <input value={r.name} onChange={e=>updRow(r.id,"name",e.target.value.slice(0,50))} maxLength={50} style={{...inp,flex:1,padding:"3px 8px",fontSize:13,minWidth:0}}/>
            <input type="text" value={r.amount||""} onChange={e=>updRow(r.id,"amount",Number(_dig(e.target.value,7))||0)} placeholder="0" maxLength={7} style={{...inp,width:120,padding:"3px 8px",fontSize:13,textAlign:"right"}}/>
        ...
        return(
          <div style={{maxWidth:960}}>
            ...
            {(()=>{
              const now=new Date();
              const moOpts=[];
              for(let i=0;i<24;i++){const d=new Date(now.getFullYear(),now.getMonth()-i,1);moOpts.push(d.getFullYear()+"-"+String(d.getMonth()+1).padStart(2,"0"));}
              const delta=(a,b)=>{if(!a&&!b)return null;const d=b-a;const pct=a?((d/a)*100):null;return{d,pct};};
              const DeltaBadge=({a,b,invert})=>{
                const r=delta(a,b);if(!r)return<span style={{color:"#94a3b8",fontSize:12}}>—</span>;
                const good=invert?r.d<0:r.d>=0;
                const col=r.d===0?"#64748b":good?"#15803d":"#dc2626";
                const bg=r.d===0?"#f1f5f9":good?"#dcfce7":"#fee2e2";
                return<span style={{background:bg,color:col,borderRadius:6,padding:"2px 8px",fontSize:12,fontWeight:700,display:"inline-flex",alignItems:"center",gap:3}}>
                  {r.d>0?"+":""}{(r.d||0).toLocaleString("ru-RU")}
                  {r.pct!=null&&<span style={{opacity:0.75,fontSize:10}}>({r.d>0?"+":""}{r.pct.toFixed(1)}%)</span>}
                </span>;
              };
              const metrics=[
                {label:"Продажи офиса",k1:"office",unit:"₽",money:true},
                {label:"Продажи агентов",k1:"agents",unit:"₽",money:true},
                {label:"Продажи менеджера",k1:"manager",unit:"₽",money:true},
                {label:"Всего объём продаж",k1:null,money:true,calc:d=>d.office+d.agents+d.manager,bold:true},
                {label:"Полисов офиса",k1:"officeCount",money:false},
                {label:"Полисов агентов",k1:"agentCount",money:false},
                {label:"Расходы офиса",k1:"expenses",money:true,invert:true},
              ];
              return(
                <div style={{background:"white",borderRadius:14,padding:"16px",boxShadow:"0 2px 12px rgba(15,23,42,0.09)",border:"1px solid #e2e8f0",marginBottom:20}}>
                  <div style={{fontWeight:700,fontSize:15,color:"#0f172a",marginBottom:14}}>📅 Сравнение месяцев</div>
                  <div style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap",marginBottom:16}}>
                    <select value={cmpMonth1} onChange={e=>{setCmpMonth1(e.target.value);setCmpData(null);}} style={{...inp,padding:"6px 10px",fontSize:13,minWidth:150}}>
                      <option value="">— Месяц 1 —</option>
                      {moOpts.map(m=><option key={m} value={m}>{fmtMonth(m)}</option>)}
                    </select>
                    <span style={{color:"#94a3b8",fontWeight:700,fontSize:16}}>vs</span>
                    <select value={cmpMonth2} onChange={e=>{setCmpMonth2(e.target.value);setCmpData(null);}} style={{...inp,padding:"6px 10px",fontSize:13,minWidth:150}}>
                      <option value="">— Месяц 2 —</option>
                      {moOpts.map(m=><option key={m} value={m}>{fmtMonth(m)}</option>)}
                    </select>
                    <button onClick={loadComparison} disabled={!cmpMonth1||!cmpMonth2||cmpLoading} style={btn("#1d4ed8",undefined,{fontSize:12,opacity:(!cmpMonth1||!cmpMonth2)?0.5:1})}>
                      {cmpLoading?"Загрузка...":"Сравнить"}
                    </button>
                    {cmpData&&<button onClick={()=>setCmpData(null)} style={{...btn("#f1f5f9","#374151",{fontSize:12}),border:"1px solid #cbd5e1"}}>✕ Сбросить</button>}
                  </div>
                  {cmpData&&(()=>{
                    const{d1,d2}=cmpData;
                    return(
                        ...
                        <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
                          <thead>
                            <tr style={{background:"#1e293b"}}>
                              <th style={{...th,color:"#e2e8f0",background:"#1e293b",minWidth:200}}>Метрика</th>
                              <th style={{...th,color:"#60a5fa",background:"#1e293b",textAlign:"right",minWidth:140}}>{fmtMonth(cmpData.m1)}</th>
                              <th style={{...th,color:"#4ade80",background:"#1e293b",textAlign:"right",minWidth:140}}>{fmtMonth(cmpData.m2)}</th>
                              <th style={{...th,color:"#e2e8f0",background:"#1e293b",textAlign:"right",minWidth:160}}>Изменение</th>
                            </tr>
                          </thead>
                          <tbody>
                            {metrics.map((row,i)=>{
                              const v1=row.calc?row.calc(d1):(d1[row.k1]||0);
                              const v2=row.calc?row.calc(d2):(d2[row.k1]||0);
                              const bg=row.bold?(i%2===0?"#f0f4ff":"#e8eeff"):(i%2===0?"white":"#f8fafc");
                              return(
                                <tr key={row.label} style={{background:bg,borderBottom:"1px solid #e2e8f0"}}>
                                  <td style={{...td,fontWeight:row.bold?700:500,color:row.bold?"#0f172a":"#374151",paddingLeft:12}}>{row.label}</td>
                                  <td style={{...td,textAlign:"right",fontWeight:row.bold?700:400,color:"#1d4ed8"}}>{row.money?(v1||0).toLocaleString("ru-RU")+" ֏":v1||0}</td>
                                  <td style={{...td,textAlign:"right",fontWeight:row.bold?700:400,color:"#15803d"}}>{row.money?(v2||0).toLocaleString("ru-RU")+" ֏":v2||0}</td>
                                  <td style={{...td,textAlign:"right"}}><DeltaBadge a={v1} b={v2} invert={row.invert}/></td>
                                </tr>
                              );
                            })}
                          </tbody>
                  ...
                  {!cmpData&&!cmpLoading&&cmpMonth1&&cmpMonth2&&<div style={{textAlign:"center",color:"#94a3b8",padding:"20px 0",fontSize:13}}>Нажмите «Сравнить» для загрузки данных</div>}
                  {!cmpData&&!cmpLoading&&(!cmpMonth1||!cmpMonth2)&&<div style={{textAlign:"center",color:"#cbd5e1",padding:"20px 0",fontSize:13}}>Выберите два месяца для сравнения</div>}
                </div>
              );
            })()}
            {/* ── ОСАГО ── */}
                    ...
                    <div style={{padding:"12px 14px 12px 20px"}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8}}>
                        <div style={{flex:1,minWidth:0}}>
                          <div style={{fontSize:14,fontWeight:600,color:"#1e293b",whiteSpace:"pre-wrap",lineHeight:1.5,marginBottom:b.reminderAt||b.policyRef?8:0,overflowWrap:"break-word"}}>{b.text}</div>
                          {b.policyRef&&(
                ...
                <div style={{padding:"14px 16px",maxHeight:"70vh",overflowY:"auto"}}>
                  ...
                  {sel.status!=="closed"&&(
                    <div style={{borderTop:"1px solid #e5e7eb",paddingTop:10}}>
                      <div style={{display:"flex",gap:6}}>
                        <input value={taskComment} onChange={e=>setTaskComment(e.target.value.slice(0,1000))} onKeyDown={e=>e.key==="Enter"&&!e.shiftKey&&addComment(sel)} placeholder="Написать комментарий..." maxLength={1000} style={{...inp,flex:1,fontSize:12,padding:"6px 10px"}}/>
      ...
      {tab==="renewals"&&(()=>{
        ...
        const missed=rntab.results?[...rntab.results.filter(r=>r._status!=="renewed"&&matchAgent(r)&&(rntab.work[rnPolKey(r)]?.operatorStatus)!=="refused"&&(rntab.work[rnPolKey(r)]?.operatorStatus)!=="sold")].sort((a,b)=>{const da=parseAnyDate(getEndDate(a));const db=parseAnyDate(getEndDate(b));if(!da&&!db)return 0;if(!da)return 1;if(!db)return-1;return da.getTime()-db.getTime();}):[];
        ...
        const sold=rntab.results?[...rntab.results.filter(r=>r._status!=="renewed"&&matchAgent(r)&&(rntab.work[rnPolKey(r)]?.operatorStatus)==="sold")].sort((a,b)=>{const da=parseAnyDate(getEndDate(a));const db=parseAnyDate(getEndDate(b));if(!da&&!db)return 0;if(!da)return 1;if(!db)return-1;return da.getTime()-db.getTime();}):[];
        ...
        return(
          <div style={{maxWidth:1300}}>
            ...
            {/* SOLD TABLE (collapsible) */}
            {rntab.results&&sold.length>0&&(
              <div style={{marginBottom:16}}>
                <button onClick={()=>rnUpdateTab(tid,{soldOpen:!rntab.soldOpen})}
                  style={{display:"flex",alignItems:"center",gap:8,background:"#fff7ed",border:"1px solid #fdba74",borderRadius:10,padding:"10px 16px",cursor:"pointer",marginBottom:rntab.soldOpen?10:0,width:"100%",textAlign:"left"}}>
                  <span style={{fontSize:14}}>{rntab.soldOpen?"▼":"▶"}</span>
                  <span style={{fontWeight:700,color:"#9a3412",fontSize:14}}>💸 Продано — {sold.length}</span>
                </button>
                {rntab.soldOpen&&(
                  <div style={{overflowX:"auto",borderRadius:10,border:"1px solid #fdba74"}}>
                    <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                        ...
                        {sold.map((p,i)=>{
                          ...
                          return(
                            <tr key={i} style={{background:i%2===0?"#fff":"#fff7ed",borderBottom:"1px solid #fed7aa"}}>
                              <td style={{...tdS,fontWeight:600,color:"#9a3412",whiteSpace:"nowrap"}}>{endDate||"—"}</td>
                              <td style={{...tdS,maxWidth:150,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{p.insuredName||"—"}</td>
                              <td style={{...tdS,fontFamily:"monospace",fontSize:11,color:"#0f766e",fontWeight:600}}>{p.phone||"—"}</td>
                              <td style={{...tdS,fontFamily:"monospace",fontWeight:700}}>{p.carPlate||"—"}</td>
                              <td style={tdS}>{p.company||"—"}</td>
                              <td style={{...tdS,color:"#1d4ed8",fontWeight:600,fontSize:11}}>{p.policyNum||"—"}</td>
                              <td style={{...tdS,textAlign:"center"}}>
                                {rnCanEdit&&<select value="sold" onChange={e=>updateRnStatus(tid,pk,e.target.value)}
                                  style={{fontSize:11,padding:"3px 6px",borderRadius:6,border:"1px solid #d1d5db",background:"#fff7ed",color:"#9a3412",fontWeight:600,cursor:"pointer",minWidth:120}}>
                                  {Object.entries(OP_STATUS).map(([k,v])=><option key={k} value={k}>{v.label}</option>)}
                                </select>}
                              </td>
                            </tr>
      ...
      {tab==="renewals_mreo"&&(()=>{
        ...
        const missed=[...((rntab.results||[]).filter(r=>r._status!=="renewed"&&matchAgent(r)&&(rntab.work[rnPolKey(r)]?.operatorStatus)!=="refused"&&(rntab.work[rnPolKey(r)]?.operatorStatus)!=="sold"))].sort((a,b)=>{const da=parseAnyDate(getEndDate(a));const db=parseAnyDate(getEndDate(b));if(!da&&!db)return 0;if(!da)return 1;if(!db)return-1;return da.getTime()-db.getTime();});
        const refused=[...((rntab.results||[]).filter(r=>r._status!=="renewed"&&matchAgent(r)&&(rntab.work[rnPolKey(r)]?.operatorStatus)==="refused"))];
        const sold=[...((rntab.results||[]).filter(r=>r._status!=="renewed"&&matchAgent(r)&&(rntab.work[rnPolKey(r)]?.operatorStatus)==="sold"))].sort((a,b)=>{const da=parseAnyDate(getEndDate(a));const db=parseAnyDate(getEndDate(b));if(!da&&!db)return 0;if(!da)return 1;if(!db)return-1;return da.getTime()-db.getTime();});
          ...
          <div style={{maxWidth:1300}}>
            ...
            {rntab.results&&sold.length>0&&(
              <div style={{marginBottom:16}}>
                <button onClick={()=>rnUpdateTab(tid,{soldOpen:!rntab.soldOpen})}
                  style={{display:"flex",alignItems:"center",gap:8,background:"#fff7ed",border:"1px solid #fdba74",borderRadius:10,padding:"10px 16px",cursor:"pointer",marginBottom:rntab.soldOpen?10:0,width:"100%",textAlign:"left"}}>
                  <span>{rntab.soldOpen?"▼":"▶"}</span>
                  <span style={{fontWeight:700,color:"#9a3412",fontSize:14}}>💸 Продано — {sold.length}</span>
                </button>
                {rntab.soldOpen&&<div style={{overflowX:"auto",borderRadius:10,border:"1px solid #fdba74"}}>
                  <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                    <thead><tr style={{background:"#ea580c"}}>{["Дата оконч.","Страхователь","Телефон","Госномер",""].map((h,i)=><th key={i} style={thS}>{h}</th>)}</tr></thead>
                    <tbody>{sold.map((p,i)=>{const pk=rnPolKey(p);return(
                      <tr key={i} style={{background:i%2===0?"#fff":"#fff7ed",borderBottom:"1px solid #fed7aa"}}>
                        <td style={{...tdS,fontWeight:600,color:"#9a3412",whiteSpace:"nowrap"}}>{p.endDateFmt||isoToDisp(p.endDate)||"—"}</td>
                        <td style={tdS}>{p.insuredName||"—"}</td>
                        <td style={{...tdS,fontFamily:"monospace",fontSize:11,color:"#0f766e"}}>{p.phone||"—"}</td>
                        <td style={{...tdS,fontFamily:"monospace",fontWeight:700}}>{p.carPlate||"—"}</td>
                        <td style={{...tdS,textAlign:"center"}}>
            ...
            {!amexLoaded
              ...
              :<>
                ...
                <div style={{background:"white",border:"1px solid #e5e7eb",borderRadius:12,padding:"14px 16px",marginBottom:16}}>
                  <div style={{fontWeight:700,fontSize:13,color:"#1d4ed8",marginBottom:10}}>💳 Добавить пополнение карты</div>
                  <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"flex-end"}}>
                    <div>
                      <div style={{fontSize:11,color:"#6b7280",marginBottom:3}}>Дата *</div>
                      <input type="date" value={amexNewTopup.date} onChange={e=>setAmexNewTopup(p=>({...p,date:e.target.value}))} style={{...inp,padding:"7px 10px",fontSize:13}}/>
                    </div>
                    <div>
                      <div style={{fontSize:11,color:"#6b7280",marginBottom:3}}>Сумма (AMD) *</div>
                      <input type="text" value={amexNewTopup.amount} onChange={e=>setAmexNewTopup(p=>({...p,amount:_dig(e.target.value,9)}))} placeholder="0" maxLength={9} style={{...inp,padding:"7px 10px",fontSize:13,width:130,textAlign:"right"}}/>
                    </div>
                    <div style={{flex:1,minWidth:140}}>
                      <div style={{fontSize:11,color:"#6b7280",marginBottom:3}}>Комментарий</div>
                      <input value={amexNewTopup.comment} onChange={e=>setAmexNewTopup(p=>({...p,comment:e.target.value.slice(0,80)}))} placeholder="Необязательно" maxLength={80} style={{...inp,padding:"7px 10px",fontSize:13,width:"100%",boxSizing:"border-box"}}/>
                    </div>
                    <button onClick={addAmexTopup} disabled={!amexNewTopup.date||!(parseFloat(amexNewTopup.amount)>0)} style={btn("#1d4ed8")}>+ Добавить</button>
                  </div>
                </div>
      ...
      {showAudit&&isAdmin&&(()=>{
        const AM={
          add_policy:{label:"Добавлен полис",icon:"📋",cat:"policy",bg:"#dcfce7",col:"#166534"},
          edit_policy:{label:"Изменён полис",icon:"✏️",cat:"policy",bg:"#dbeafe",col:"#1d4ed8"},
          delete_policy:{label:"Удалён полис",icon:"🗑",cat:"policy",bg:"#fee2e2",col:"#991b1b"},
          import:{label:"Импорт",icon:"📥",cat:"policy",bg:"#ede9fe",col:"#6d28d9"},
          pay_policy:{label:"Оплата принята",icon:"💵",cat:"finance",bg:"#ccfbf1",col:"#0f766e"},
          cash_close:{label:"Касса закрыта",icon:"🔐",cat:"finance",bg:"#fef3c7",col:"#92400e"},
          cash_reopen:{label:"Касса открыта",icon:"🔓",cat:"finance",bg:"#fef9c3",col:"#a16207"},
          expense_change:{label:"Расходы",icon:"📊",cat:"finance",bg:"#f0fdf4",col:"#15803d"},
          amex_topup:{label:"Пополнение Amex",icon:"💳",cat:"amex",bg:"#eff6ff",col:"#1d4ed8"},
          amex_topup_delete:{label:"Удалено пополнение",icon:"💳",cat:"amex",bg:"#fee2e2",col:"#991b1b"},
          lock_month:{label:"Закрыт месяц",icon:"🔒",cat:"month",bg:"#ffedd5",col:"#9a3412"},
          unlock_month:{label:"Открыт месяц",icon:"🔓",cat:"month",bg:"#fef9c3",col:"#92400e"},
          settings:{label:"Настройки",icon:"⚙️",cat:"settings",bg:"#f1f5f9",col:"#475569"},
          dir_change:{label:"Справочник агентов",icon:"👤",cat:"settings",bg:"#f0fdf4",col:"#15803d"},
          login:{label:"Вход",icon:"🔑",cat:"auth",bg:"#ecfdf5",col:"#047857"},
          logout:{label:"Выход",icon:"🚪",cat:"auth",bg:"#f8fafc",col:"#64748b"},
        };
        const CAT_LABELS={policy:"📋 Полисы",finance:"💰 Финансы",amex:"💳 Amex",month:"🔒 Месяц",settings:"⚙️ Настройки",auth:"🔑 Вход/Выход"};
        const allUsers=[...new Set(["Администратор",...employees.map(e=>e.name),...auditEntries.map(e=>e.user)])].sort();
        const filtered=auditEntries.filter(e=>{
          if(auditFilter.user&&e.user!==auditFilter.user)return false;
          if(auditFilter.category&&(AM[e.action]?.cat||"settings")!==auditFilter.category)return false;
          if(auditFilter.dateFrom){const d=new Date(e.ts).toISOString().slice(0,10);if(d<auditFilter.dateFrom)return false;}
          if(auditFilter.dateTo){const d=new Date(e.ts).toISOString().slice(0,10);if(d>auditFilter.dateTo)return false;}
          if(auditFilter.search&&!(e.details||"").toLowerCase().includes(auditFilter.search.toLowerCase())&&!(e.user||"").toLowerCase().includes(auditFilter.search.toLowerCase()))return false;
          return true;
        });
        const hasFilter=auditFilter.user||auditFilter.category||auditFilter.dateFrom||auditFilter.dateTo||auditFilter.search;
        ...
        filtered.forEach(e=>{
          ...
          if(!byDate[day][e.user])byDate[day][e.user]=[];
          byDate[day][e.user].push(e);
        });
        ...
        return(
          <>
            ...
            <div style={{position:"fixed",right:0,top:0,width:680,height:"100vh",background:"#f8fafc",zIndex:1001,display:"flex",flexDirection:"column",boxShadow:"-8px 0 40px rgba(15,23,42,0.25)"}}>
              {/* Шапка */}
              <div style={{background:"#0f172a",padding:"14px 20px",display:"flex",justifyContent:"space-between",alignItems:"center",flexShrink:0}}>
                <div>
                  <div style={{color:"#fff",fontWeight:700,fontSize:16}}>📋 Журнал действий</div>
              ...
              {/* Фильтры */}
              <div style={{padding:"10px 14px",borderBottom:"1px solid #e2e8f0",background:"white",flexShrink:0}}>
                <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:6}}>
                  ...
                  <select value={auditFilter.category} onChange={e=>setAuditFilter(p=>({...p,category:e.target.value}))} style={{...inp,fontSize:12,padding:"4px 8px",flex:1,minWidth:130}}>
                    <option value="">Все категории</option>
                    {Object.entries(CAT_LABELS).map(([k,v])=><option key={k} value={k}>{v}</option>)}
                  </select>
                </div>
                <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                  <input type="date" value={auditFilter.dateFrom} onChange={e=>setAuditFilter(p=>({...p,dateFrom:e.target.value}))} style={{...inp,fontSize:12,padding:"4px 8px"}} title="С даты"/>
                  <input type="date" value={auditFilter.dateTo} onChange={e=>setAuditFilter(p=>({...p,dateTo:e.target.value}))} style={{...inp,fontSize:12,padding:"4px 8px"}} title="По дату"/>
                  <input value={auditFilter.search} onChange={e=>setAuditFilter(p=>({...p,search:e.target.value}))} placeholder="🔍 Поиск по тексту..." style={{...inp,fontSize:12,padding:"4px 8px",flex:1,minWidth:140}}/>
                  {hasFilter&&<button onClick={()=>setAuditFilter({user:"",category:"",dateFrom:"",dateTo:"",search:""})} style={{...btn("#f1f5f9","#374151",{fontSize:11,padding:"4px 10px"}),border:"1px solid #cbd5e1"}}>✕ Сброс</button>}
                </div>
              </div>
              {/* Тело */}
              <div style={{flex:1,overflowY:"auto",padding:"12px 14px"}}>
                {filtered.length===0
                  ...
                  :sortedDates.map(day=>{
                    ...
                    const dayTotal=Object.values(dayEntries).reduce((s,arr)=>s+arr.length,0);
                    const dayLabel=fmtDay(new Date(day+"T12:00:00"));
                    return(
                      <div key={day} style={{marginBottom:16}}>
                        {/* Заголовок дня */}
                        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                          <div style={{fontWeight:700,fontSize:13,color:"#0f172a",textTransform:"capitalize"}}>{dayLabel}</div>
                          <span style={{background:"#e2e8f0",color:"#64748b",borderRadius:10,padding:"1px 8px",fontSize:11,fontWeight:600}}>{dayTotal}</span>
                          <div style={{flex:1,height:1,background:"#e2e8f0"}}/>
                        </div>
                        {/* Группы по сотрудникам */}
                        {Object.entries(dayEntries).sort(([a],[b])=>a.localeCompare(b)).map(([user,entries])=>(
                          <div key={user} style={{marginBottom:10,background:"white",borderRadius:10,border:"1px solid #e5e7eb",overflow:"hidden"}}>
                            {/* Подзаголовок сотрудника */}
                            <div style={{background:"#f8fafc",padding:"6px 12px",display:"flex",alignItems:"center",gap:6,borderBottom:"1px solid #f1f5f9"}}>
                              <span style={{fontSize:13}}>👤</span>
                              <span style={{fontWeight:700,fontSize:12,color:"#1e293b"}}>{user}</span>
                              <span style={{background:"#e2e8f0",color:"#64748b",borderRadius:10,padding:"1px 7px",fontSize:10,fontWeight:600,marginLeft:2}}>{entries.length}</span>
                            </div>
                            {/* Записи */}
                            {entries.map((e,i)=>{
                              const m=AM[e.action]||{label:e.action,icon:"•",bg:"#f1f5f9",col:"#374151"};
                              return(
                                <div key={e.id||i} style={{padding:"7px 12px",borderTop:i>0?"1px solid #f8fafc":"none",display:"flex",gap:8,alignItems:"flex-start"}}>
                                  <span style={{fontSize:11,color:"#94a3b8",whiteSpace:"nowrap",marginTop:1,minWidth:36}}>{fmtTime(e.ts)}</span>
                                  <span style={{background:m.bg,color:m.col,borderRadius:5,padding:"1px 6px",fontSize:10,fontWeight:700,whiteSpace:"nowrap",flexShrink:0}}>{m.icon} {m.label}</span>
                                  <span style={{fontSize:12,color:"#374151",flex:1,minWidth:0,wordBreak:"break-word",lineHeight:1.4}}>{e.details}</span>
                                  {e.month&&e.month!=="—"&&<span style={{fontSize:10,color:"#94a3b8",whiteSpace:"nowrap",flexShrink:0,marginTop:1}}>{fmtMonth(e.month)}</span>}
                                </div>
                              );
                            })}
                          </div>
              ...
              <div style={{padding:"8px 14px",borderTop:"1px solid #e2e8f0",background:"#f1f5f9",fontSize:11,color:"#64748b",flexShrink:0}}>
                Хранится 1000 последних записей · Все действия всех пользователей
              </div>
            </div>
          </>
        );
      })()}
      {rnDetailPol&&<RnPolModal p={rnDetailPol} onClose={()=>setRnDetailPol(null)} rnTabs={rnTabs} rnActiveId={rnActiveId} parseAnyDate={parseAnyDate} getName={getName}/>}
      {showNotifs&&(
        <>
